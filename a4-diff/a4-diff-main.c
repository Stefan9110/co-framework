...
bool has_file_permissions(const char *file_name) {
  return access(file_name, F_OK) == 0;
}

void get_options(int argc, char **argv, bool *flag_i, bool *flag_B) {
  assert(argc >= 3);
  assert(argv);
  assert(flag_i);
  assert(flag_B);
...
int main(int argc, char **argv) {
  ...
  if (!has_file_permissions(argv[1])) {
    fprintf(stderr, "[ERROR] could not open file \"%s\".\n", argv[1]);
    return 1;
  }
  if (!has_file_permissions(argv[2])) {
    fprintf(stderr, "[ERROR] could not open file \"%s\".\n", argv[2]);
    return 1;
  }
